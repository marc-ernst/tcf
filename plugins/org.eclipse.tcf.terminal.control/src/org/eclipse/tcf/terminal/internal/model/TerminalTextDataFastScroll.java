...
public class TerminalTextDataFastScroll implements ITerminalTextData {
    ...
    public TerminalTextDataFastScroll(ITerminalTextData data, int maxHeight) {
        ...
        if (maxHeight > 2) assert shiftOffset(-2) || throwRuntimeException();
    }
    ...
    public void setDimensions(int height, int width) {
        ...
        if (height > fMaxHeight) setMaxHeight(height);
        fHeight = height;
        if (width != fData.getWidth()) fData.setDimensions(fMaxHeight, width);
    }
    ...
    public void setMaxHeight(int maxHeight) {
        ...
        if (start != 0) {
            ...
            if (n < maxHeight) buffer.copyRange(fData, 0, n, Math.min(fMaxHeight - n, maxHeight - n));
