...
import org.eclipse.tcf.terminal.model.ITerminalTextDataReadOnly;
...
abstract public class AbstractTextCanvasModel implements ITextCanvasModel {
    ...
    protected void updateSnapshot() {
        if (!fInUpdate && fSnapshot.isOutOfDate()) {
            ...
            try {
                ...
                if (fSnapshot.hasTerminalChanged()) fireTerminalDataChanged();
    ...
    public Point getSelectionAnchor() {
        if (fSelectionStartLine < 0) return null;
    ...
    private String extractSelectedText() {
        if (fSelectionStartLine < 0 || fSelectionStartCoumn < 0 || fSelectionSnapshot == null) return ""; //$NON-NLS-1$
        ...
        for (int line = fSelectionStartLine; line <= fSeletionEndLine; line++) {
            ...
            if (chars != null) {
                ...
                if (line == fSelectionStartLine) text = text.substring(Math.min(fSelectionStartCoumn, text.length()));
            ...
            buffer.append(text);
            if (line < fSeletionEndLine && !fSelectionSnapshot.isWrappedLine(line)) buffer.append('\n');
        }
