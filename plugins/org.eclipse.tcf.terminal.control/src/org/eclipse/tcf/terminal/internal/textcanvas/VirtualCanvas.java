...
public abstract class VirtualCanvas extends Canvas {
    ...
    protected boolean inClipping(Rectangle clipping, Rectangle r) {
        ...
        //
        if (r.x + r.width <= clipping.x) return false;
        if (clipping.x + clipping.width <= r.x) return false;
        if (r.y + r.height <= clipping.y) return false;
        if (clipping.y + clipping.height <= r.y) return false;
